...
class MySQL_Backup
{
  ...
  function Execute($task = MSB_STRING, $fname = '', $compress = false)
  {
    ...
    if ($task == MSB_SAVE)
    {
      if (empty($fname))
      {
        ...
        $fname .= date($this->fname_format).'_'.$this->itenablever;
  ...
  function _Connect()
  {
    ...
    if ($this->link_id)
    {
      ...
      {
		mysql_query('set names '.$this->Read_Char);
  ...
  function _Retrieve()
  {
    ...
    if ($this->comments)
    {
      ...
      $value .= '# Created by ItEnable Mysql Backup System'. MSB_NL;
      $value .= '#' . MSB_NL;
