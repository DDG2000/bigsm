...
$Admin->CheckPopedoms('SM_ITEM_FACTORY');
...
$ItemOb = new ItemFactory();
switch($Work)
 {
 	case 'MdyReco': /**编辑**/
		 $Admin->CheckPopedoms('SM_ITEM_FACTORY_MDY');
	     GetValue($FLib);
...
function createitemclasscache(){
    ...
    global $DataBase;
    $itemFactoryArray = $DataBase->fetch_all_assoc("select * from sm_item_factory where Status=1 and I_mall_classID=1  order by I_order asc") ;
    $data = CacheManager::saveCache(CACHE_ITEM_FACTORY, $itemFactoryArray) ;
    if($data==false){
        return array('flag'=>0);
    }
    return array('flag'=>1);
