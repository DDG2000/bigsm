...
// 		 $msg = $r['flag']<1 ? ',生成失败!'.$r['err']:'';
	...
	case 'AddReco': /**增加**/
		 ...
		 $Item->addItem($I_mall_classID, $name, $order,$itemclassid);
		 
		 $r = createitemclasscache();
		 
		 $msg = $r['flag']<1 ? ',生成失败!':'';
		 $Admin ->AddLog($mallclass.$tt,'增加品名：其名称为：'.$name);
...
//                  		     exit();S
                     ...
                     if ($k != 0)
                     {
                         if($k>1){
                            ...
                            // $result=$ItemClass->addItemClass($I_mall_classID, $name, $order);
                             $result=$Item->addItem($I_mall_classID, $name, $order,$itemclassid);
                             $Admin ->AddLog($mallclass.$tt,'增加品名：其名称为：'.$name);
    ...
    case 'DeleteReco': /**删除**/
	     ...
	     $Item->deleteItem($IdList);
	
		 $r = createitemclasscache();
// 		 $msg = $r['flag']<1 ? ',生成失败!'.$r['err']:'';
		 $msg = $r['flag']<1 ? ',生成失败!':'';
		 $Admin ->AddLog($mallclass.$tt,'删除品名：其ID为：'.$IdList);
...
function createitemclasscache(){
    ...
    global $DataBase;
    $itemArray = $DataBase->fetch_all_assoc("select * from sm_item where status=1 and I_mall_classID=1  order by I_order asc") ;
    $data = CacheManager::saveCache(CACHE_ITEM, $itemArray) ;
    if($data==false){
        return array('flag'=>0);
    }
    return array('flag'=>1);
