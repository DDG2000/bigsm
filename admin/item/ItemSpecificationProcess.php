...
$ItemOb = new ItemSpecification();
switch($Work)
 {
 	case 'MdyReco': /**编辑**/
		 $Admin->CheckPopedoms('SM_ITEM_SPECIFICATION_MDY');
	     GetValue($FLib);
...
function createitemclasscache(){
    ...
    global $DataBase;
    $itemSpecificationArray = $DataBase->fetch_all_assoc("select * from sm_item_specification where status=1 and I_mall_classID=1  order by I_order asc") ;
    $data = CacheManager::saveCache(CACHE_ITEM_SPECIFICATION, $itemSpecificationArray) ;
    if($data==false){
        return array('flag'=>0);
    }
    return array('flag'=>1);
