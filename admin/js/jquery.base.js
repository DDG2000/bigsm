...
$(function(){
	if(!$("#main").hasClass("not_set")){//添加滚动条  add by sakura 20141106
		if($("#main .table_np").height()>400){
			var o_table_np = $("#main .table_np");
			var tr_bt_html = "<div class='o_divtable_1' style='background-color:#e6e6e6;padding-right:25px'><table><tr class='tr_bt' style='white-space:nowrap;'>"+o_table_np.find(".tr_bt").html()+"</tr></table></div>";
			o_table_np.find(".tr_bt").remove();
			o_table_np.html(tr_bt_html+"<div class='o_divtable_2' style='height: 400px;overflow-x:hidden;overflow-y:scroll;'>"+o_table_np.html()+"</div>");
			//jiaz
			var o_table_1 = o_table_np.find("table").eq(0);
			var o_table_2 = o_table_np.find("table").eq(1);
			o_table_2.width(o_table_1.width());
			if(o_table_2.find("tr").eq(0).find("td").length >1){
				var top_w,bot_w;
				o_table_1.find("tr").eq(0).find("td").each(function(i){
					top_w = $(this);
					bot_w = o_table_2.find("tr").eq(0).find("td").eq(i);
					if($(top_w).attr("width") != ""){
						bot_w.width(top_w.width());
					}else if($(bot_w).attr("width") != ""){
						top_w.width(bot_w.width());
					}
				});
				o_table_1.find("tr").eq(0).find("td").each(function(i){
					top_w = $(this);
					bot_w = o_table_2.find("tr").eq(0).find("td").eq(i);
					if(top_w.width() > bot_w.width()){
						bot_w.width(top_w.width());
					}else{
						top_w.width(bot_w.width());
					}
	...
	$(".del").click(function(){
		...
		_t.attr("href", _t.attr("old")+IdList+"&obj=self");
		if(_t.attr("href").toLowerCase().indexOf("delete")>-1){
			var title=typeof(_t.attr("title"))=='undefined'?'':'<br>'+_t.attr("title"),
				info = '是否确认['+_t.html()+']选中项'+title;
	...
	$("form[check]").submit(function(){
		...
		$(this).find(":input[isc]").each(function(){
			...
			if(val == "" && _t.attr("ennull")==undefined && _t.attr("nisc")==undefined){
				...
				if( ( isc.substr(0,3)=='int' && (r=isc.match(/\[(\d+)-([\d|\*]+)\]/)) ) 
					|| (isc.substr(0,7)=='dynamic' && (r=isc.match(/\[([\w\.\-\.#.]+),([\w|\-\.#.]+)\]/)))
		...
		if(valid){
			var isNeedConfirm = $(this).find("input[name=isNeedConfirm]").val();
			if (isNeedConfirm) {//好利贷需要这货 这有意思吗 20141125
				if (confirm("确认提交？")) {
					_form.attr("issubmit","yes");
				} else {
					valid = false;
				}
			} else {
