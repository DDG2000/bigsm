 ...
 * @return array
 */
function  get_goodstree(){
//     cache ( 'GoodsTree',null );
    $goodstreeArr = cache ( 'GoodsTree' );
    if (! $goodstreeArr) {
    
    $goodtree = array();
    $g_type = db('erp_goods_tree')->where("Vc_goods_type!='其他'")->field('Vc_goods_type')->group('Vc_goods_type')->select();//类型
    
    foreach ($g_type as $key=>$val){
        $temp['id'] = $key+1;
        $temp['pId'] = 0;
        $temp['name'] = $val['Vc_goods_type'];
        $temp['nocheck'] = true;
        $goodtree[] =$temp;
    }
    $type = $goodtree;
    foreach ($type as $k=>$v0){
         
        $g_class = db('erp_goods_tree')
        ->where("Vc_goods_type='{$v0['name']}' and Vc_goods_class!='其他'")
        ->field('Vc_goods_class')
        ->group('Vc_goods_class')->select();//大类
         
        foreach ($g_class as $k1=>$v){
            $class['id'] =count($goodtree)+$k+1;
            $class['pId'] = $v0['id'];
            $class['name'] = $v['Vc_goods_class'];
            $class['nocheck'] = true;
            $goodtree[] =$class;
             
            $g_breed = db('erp_goods_tree')
            ->where("Vc_goods_type='{$v0['name']}' and Vc_goods_class='{$v['Vc_goods_class']}'")
            ->field('Vc_goods_breed')
            ->group('Vc_goods_breed')
            ->select();//品名
            foreach ($g_breed as $k2=>$v2){
                 
                $breed['id'] =count($goodtree)+ $k+1;
                $breed['pId'] = $class['id'];
                $breed['name'] = $v2['Vc_goods_breed'];
                $breed['nocheck'] = true;
                $goodtree[]=$breed;
    
                $g_material = db('erp_goods_tree')
                ->where("Vc_goods_type='{$v0['name']}' and Vc_goods_class='{$v['Vc_goods_class']}' and Vc_goods_breed='{$v2['Vc_goods_breed']}'")
                ->field('Vc_goods_material')
                ->group('Vc_goods_material')
                ->select();//材质
                 
                 
                foreach ($g_material as $k3=>$v3){
    
                    $material['id'] =count($goodtree)+ $k+1;
                    $material['pId'] = $breed['id'];
                    $material['name'] = $v3['Vc_goods_material'];
                    $material['nocheck'] = true;
                    $goodtree[]=$material;
    
                    $g_spec = db('erp_goods_tree')
                    ->where("Vc_goods_type='{$v0['name']}' and Vc_goods_class='{$v['Vc_goods_class']}' and Vc_goods_breed='{$v2['Vc_goods_breed']}' and Vc_goods_material='{$v3['Vc_goods_material']}'")
                    ->field('Vc_goods_code,Vc_goods_spec')
                    ->group('Vc_goods_spec')
                    ->select();//规格
    
                    foreach ($g_spec as $k4=>$v4){
                         
                        $spec['id'] =count($goodtree)+ $k+1;
                        $spec['pId'] = $material['id'];
                        $spec['name'] = $v4['Vc_goods_spec'];
                        // 	 $spec['nocheck'] = true;
                        $spec['code'] = $v4['Vc_goods_code'];
                        $spec['info'] =$v2['Vc_goods_breed'].",".$v3['Vc_goods_material'].",". $v4['Vc_goods_spec'];
                        $goodtree[]=$spec;
    
//                         $g_factory = db('erp_goods_tree')
//                         ->where("Vc_goods_type='{$v0['Vc_goods_type']}' and Vc_goods_class='{$v['Vc_goods_class']}' and Vc_goods_breed='{$v2['Vc_goods_breed']}'  and Vc_goods_material='{$v3['Vc_goods_material']}'  and Vc_goods_spec='{$v4['Vc_goods_spec']}' ")
//                         ->field('Vc_goods_code,Vc_goods_factory')
//                         ->group('Vc_goods_factory')
//                         ->select();//钢厂

//                         foreach ($g_factory as $k5=>$v5){

//                             $factory['id'] =count($goodtree)+ $k+1;
//                             $factory['pId'] = $spec['id'];
//                             $factory['name'] = $v5['Vc_goods_factory'];
//                             $factory['code'] = $v5['Vc_goods_code'];
//                             $goodtree[]=$factory;
...
//                         }
    ...
    $goodstreeArr = cache('GoodsTree', $goodtree);
 ...
 * 得到预估价格
 */
function get_judgeprice(){
    
 
    $goodsJudgePrice = db('configure')->where("code='GoodsJudgePrice'")->cache('GoodsJudgePrice',3600)->value('value');
      
    return $goodsJudgePrice;
...
function get_malls(){
    ...
    $malls = db('sm_industry')->where('state=1')->cache('industry_data',3600)->select();
    return $malls;
