...
$.fn.extend({
	sSelect: function() {
		...
		var tabindex = -1;//this.tabIndex||0;
		...
		var selectWidth=$('#'+selectId+' select').width(),selectUlHeight=$('#'+selectId+' select').attr("ulheight");
		//if($.browser.safari){selectWidth = selectWidth}
		$('#'+selectId).css({'width':selectWidth+8});
		...
		if(selectUlHeight!=undefined){$('#'+selectId+' ul').attr({"maxHeight":selectUlHeight})}
	...
	clearSelectMenu:function(selectId,selectZindex){
		if(selectId != undefined){
			...
			if(selectZindex=='auto') selectZindex=0;
			$('#'+selectId+' ul').hide();//.empty()
			$('#'+selectId+' h4').removeClass("over").removeClass("current");
			$('#'+selectId).removeClass("active_jqs");
		}
	},
	setSelectValue:function(sID){
		...
		$('#'+sID+' ul').html(content);
		var maxheight=200,_mh=$('#'+sID+' ul').attr("maxHeight");
		if(_mh!=undefined){maxheight=_mh;}
		if($('#'+sID+' ul').height()>maxheight){$('#'+sID+' ul').css({"height":maxheight,"overflowY":"scroll"})}
